    ...
    <Section id="pricing" className="py-12 md:py-24 px-4 overflow-visible">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative bg-[#0a1120] rounded-[2rem] md:rounded-[3rem] border border-slate-800 p-6 sm:p-10 md:p-20 shadow-[0_0_80px_rgba(0,0,0,0.6)] overflow-visible flex flex-col items-center text-center">
          ...
          <div className="w-full max-w-lg relative z-10 mb-10 md:mb-16 flex flex-col items-center">
            <div className="flex items-center gap-2 text-amber-500 font-bold text-sm md:text-lg mb-2">
              <Gift className="w-5 h-5 md:w-6 md:h-6 shrink-0" />
              <span>Bónus exclusivo: 10 estratégias de Rankeamento Rápido, focado nos primeiros 30 dias + suporte exclusivo por 45 dias no WhatsApp.</span>
            </div>
            <p className="text-slate-400 text-[10px] md:text-sm italic mb-4">
              para antecipar resultados e gerar tração imediata no Google Local.
            </p>
            
            <div className="w-full bg-red-600/10 border border-red-600/20 px-4 py-3 rounded-xl flex items-center justify-center gap-3">
              <Clock className="w-4 h-4 md:w-5 md:h-5 text-red-600 animate-pulse" />
              <p className="text-red-500 font-black text-xs md:text-base uppercase tracking-tight">
                Esta oferta acaba em <span className="font-mono text-sm md:text-lg">{formatTime(timeLeft)}</span>. Ao sair da página, o bônus expira.
              </p>
            </div>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-y-5 gap-y-5 gap-x-8 md:gap-x-12 text-[10px] md:text-sm text-slate-400 font-bold uppercase tracking-[0.15em] relative z-10 w-full pt-8 md:pt-10 border-t border-slate-800/50">
